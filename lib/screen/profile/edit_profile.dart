...
class _EditNewProfileState extends State<EditNewProfile> {
  ...
  uploadAadharBackFromCamOrGallary(BuildContext context) {
    containerForSheet<String>(
      context: context,
      child: CupertinoActionSheet(
        actions: <Widget>[
  ...
  Future<void> getAadharBackFromCamera() async {
    ...
    if (pickedFile != null) {
      setState(() {
        aaddarbackImage = File(pickedFile.path);
        // imagePath = File(pickedFile.path) ;
        // filePath = imagePath!.path.toString();
      });
    }
  ...
  UpdateRequest() async {
   ...
   profileImage == null ? null :   request.files.add(await http.MultipartFile.fromPath('profile_image', profileImage!.path.toString()));
  ...
  //       await http.MultipartFile.fromPath('adhar_card', aadharImage!.path.toString()));
    ...
    aadharImage == null ? null : request.files.add(await http.MultipartFile.fromPath('adhar_card', aadharImage!.path.toString()));

    aaddarbackImage == null ? null : request.files.add(await http.MultipartFile.fromPath('adhar_back', aaddarbackImage!.path.toString()));

   panImage == null ? null : request.files.add(await http.MultipartFile.fromPath('pancard', panImage!.path.toString()));
    ...
    if (response.statusCode == 200) {
      ...
      print("profileee imageeee $profileImage");
