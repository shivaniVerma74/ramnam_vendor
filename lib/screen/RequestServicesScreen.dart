...
class _RequestserviceScreenState extends State<RequestserviceScreen> {
  ...
  Future getServices() async {
    ...
    var request =
    http.MultipartRequest('POST', Uri.parse('${Apipath.getRequestBookingUrl}'));
    request.fields.addAll({'v_id': '${vid.toString()}'});
    print("this is request !! ${request.fields} and ${Apipath.getRequestBookingUrl}");
    http.StreamedResponse response = await request.send();
    print("this response @@ ${response.statusCode}");
    if (response.statusCode == 200) {
      final str = await response.stream.bytesToString();
  ...
  Widget serviceCardWidget(model, index){
    return InkWell(
      ...
      //     serviceId: model.products![index].id.toString(),
      //   )));
      // },
      child: Padding(
        padding: const EdgeInsets.only(
