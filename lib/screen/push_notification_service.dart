...
class PushNotificationService {
  ...
  Future initialise() async {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      // App.localStorage.setBool("notStatus", true);
      // notificationStatus = App.localStorage.getBool("notStatus")!;
      print("0k ${message} and ${message.notification}");
      var data = message.notification!;
      var title = data.title.toString();
      var body = data.body.toString();
      var test = message.data;
      // var newBookingId = data.bookingId.toString();
       notiType  = test['type'];
      // print('this is booking ID =====>  ${test['booking_id']} ${data} and ${notiType}  and---- ${body}  and---- ${title} ----- ${test} ooooooooo ${data.bookingId} pppppppp ');
      String bookingId = test['booking_id'];
      print("kkkkkkkkkkkkk ${bookingId}");
      var notiData =  {
        'bookingId': bookingId,
        'type' : notiType
      };
      // if(newBookingId == "" || newBookingId == null){
      //
      // }
      // else{
      //   print("working herer now ");
      //   Navigator.pushAndRemoveUntil(context,MaterialPageRoute(builder: (context) => HomeScreen(bookingId: newBookingId.toString(),)), (route) => false);
      // }
     ///
      //  onResult!(notiData);
      // }
...
Future<void> generateImageNotication(
  ...
  NotificationDetails(android: androidPlatformChannelSpecifics);
  await flutterLocalNotificationsPlugin
      .show(0, title, msg, platformChannelSpecifics, payload:title);
}
